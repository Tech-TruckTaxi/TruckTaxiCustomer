...
const ProfileScreen = ({navigation}) => {
    ...
    AsyncStorage.getItem('user').then(mobile => {
      AsyncStorage.getItem('userToken').then(value => {
        ...
        fetch(
          ...
          .then(result => {
            console.log('USERDATASSSSSS', result?.data[0]);
            setData({
              customertype: result.data[0].customertype,
              ...
              language: result.data[0].applanguage,
            });
        ...
        fetch(
          ...
          .then(response => response.json())
          .then(result => {
            console.log('BBBBHHHHhGGGGGBBB', result.data);
            setshowLoading(false);
      ...
      fetch('https://trucktaxi.co.in/api/customer/addprofile', requestOptions)
        ...
        .then(result => {
          if (result.status == 200) {
            ToastAndroid.show(
              'Profile Updated Successfully, Reloading App',
              ToastAndroid.SHORT,
            );
            var status = 'Old';
  ...
  useEffect(() => {
    navigation.setOptions({
      title: <Text>Update Profile</Text>,
    });
  });

  const loadcities = () => {
    return cityType.map(city => <SelectItem title={city.name} />);
  };
        ...
        <Animatable.View animation="fadeInUpBig" style={styles.footer}>
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{width: '100%', alignItems: 'center'}}>
              ...
              <View
                style={{width: '95%', paddingBottom: 10, marginVertical: 10}}>
                <Text
                  style={{
                    fontSize: 16,
                    color: Color.cloudyGrey,
              ...
              <View style={{width: '95%', paddingBottom: 10}}>
                ...
                <View
                  ...
                  <RadioGroup
                    selectedIndex={data?.customertype - 1}
                    onChange={index => {
                      const add = parseInt(index) + 1;
                      setSelectedIndex(index),
                        setData({...data, customertype: add});
                    }}>
                    <Radio>General</Radio>
